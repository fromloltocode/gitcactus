...
/// Protects the UI from repos with thousands of commits ahead of target.
const MAX_PREVIEW_COMMITS: usize = 100;

/// Outcome of a preview attempt. Every non-`Ready` variant is a reason the
/// UI should block or warn rather than show a commit list.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum PreviewKind {
    /// Preview computed successfully — see `commits`.
    Ready,
    /// The source branch is already reachable from target (nothing to replay).
    NoCommitsToReplay,
    /// User selected the same branch they are currently on.
    SameBranch,
    /// HEAD is detached — no current branch to rebase.
    DetachedHead,
    /// The requested target branch doesn't exist locally.
    BranchMissing,
    /// Repository has no commits yet.
    EmptyRepo,
    /// Path does not resolve to a git repository.
    NotARepo,
    /// Another error (e.g. revwalk failure). Carries a message.
    Error(String),
}
...
pub struct PreviewCommit {
    ...
    pub summary: String,
    pub author: String,
    /// Whether this commit has more than one parent — rebases handle merges
    /// specially and we want to flag it.
    pub is_merge: bool,
}
...
pub struct RebasePreview {
    /// Source branch name (current HEAD).
    pub source: String,
    /// Target branch name the user picked.
    pub target: String,
    /// Source branch tip (abbreviated).
    pub source_tip: String,
    /// Target branch tip (abbreviated).
    pub target_tip: String,
    /// Common ancestor tip (merge base) if computed, abbreviated.
    pub merge_base: Option<String>,
    /// Commits that would be replayed onto the target, oldest-first.
    pub commits: Vec<PreviewCommit>,
    /// Whether `commits` was truncated.
    pub truncated: bool,
    /// Whether any of the commits in range is a merge commit.
    pub has_merge_commits: bool,
    /// Whether the working tree has uncommitted changes at preview time.
    pub dirty_tree: bool,
    /// Why the preview is in this state.
    pub kind: PreviewKind,
}

impl RebasePreview {
    /// Build a failure preview that still carries source/target names for UI.
...
/// Compute a read-only preview of rebasing the current HEAD onto `target`.
///
/// This never mutates the repository.
pub fn preview_rebase(repo_path: &str, target: &str) -> RebasePreview {
    ...
    if revwalk.push(source_oid).is_err() {
        return RebasePreview::failure(
            &source_name,
            target,
            PreviewKind::Error("Could not push source OID".into()),
        );
    }
    if revwalk.hide(target_oid).is_err() {
        return RebasePreview::failure(
            &source_name,
            target,
            PreviewKind::Error("Could not hide target OID".into()),
        );
    }
    ...
    // revwalk returns newest-first. Reverse so UI shows oldest-first (replay order).
    commits.reverse();
